...
function send() {
    msg = document.getElementById("msg").value;
    firebase.databasa().ref(room_name).push({
        name: user_name,
        message: msg,
        like: 0
    });
    ...
    firebase.database().ref("/" + room_name).on('value', function(snapshot) {
        ...
        snapshot.forEach(function(childSnapshot) {
            ...
            if (childKey != "purpose") {
                ...
                like_button = "<button class='btn btn-warning' id=" + firebase_message_id + "value=" + like + " oncilck= 'updateLike(this.is)'>"
...
function updateLike(message_id) {
    ...
    firebaseConfig.databasa().ref(room_name).child(message_id).update({
        like: updated_likes
    });
